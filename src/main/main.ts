...
const importZip = async (filePath: string, game: string) => {
    ...
    while (videoDirectoryStack.length > 0) {
        ...
        let foundEntry: any = Object.values(entries).find((entry: any) => entry.name.toLowerCase().startsWith(videoSearchDirectory.toLowerCase()));
        if (foundEntry) {
            videoSource = foundEntry.name.substring(0, foundEntry.name.lastIndexOf("/"));
            break;
        }
    ...
    while (subtitleDirectoryStack.length > 0) {
        ...
        let foundEntry: any = Object.values(entries).find((entry: any) => entry.name.toLowerCase().startsWith(subtitleSearchDirectory.toLowerCase()));
        if (foundEntry) {
            subtitleSource = foundEntry.name.substring(0, foundEntry.name.lastIndexOf("/"));
            break;
        }
    ...
    const targetVideoDirectory = `${gameDirectory}/${videoCorrectionStack.join("/")}${videoDirectoryStack.join("/")}`;
    const targetSubtitlesDirectory = `${gameDirectory}/${subtitleCorrectionStack.join("/")}${subtitleDirectoryStack.join("/")}`;
    ...
    const videoIdList = Object.values(entries).filter((entry: any) => entry.name.toLowerCase().startsWith(sourceVideoDirectory.toLowerCase()) && entry.name.endsWith(".mp4")).map((entry: any) => entry.name.substring(entry.name.lastIndexOf("/") + 1, entry.name.lastIndexOf(".mp4")));
    ...
    videoIdList.forEach(videoId => {
        ...
        if (fs.existsSync(`${clipsDirectory}/${videoId}.mp4`) && fs.existsSync(`${subsDirectory}/${videoId}.srt`)) {
    ...
    addToCollection(game, collectionId, videoIdList.filter((videoId: string) => !mismatchedIds.includes(videoId)).map(videoId => {
        if (videoId.startsWith("_")) {
            return videoId;
        }
        return `_${videoId}`;
...
ipcMain.handle('deleteCollection', (event, { collectionId, game, deleteFiles }) => {
